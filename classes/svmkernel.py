...
def smoP(dataMatIn, classLabels, C, toler, maxIter, kTup=('lin', 0)):  # kTup初始输入值
    ...
    while (iter < maxIter) and ((alphaPairsChanged > 0) or (entireSet)):
        ...
        if entireSet:
            entireSet = False  # 如果执行了所有，且值有改变则需要执行边界，如果执行了所以值没有改变，退出。
        elif (alphaPairsChanged == 0):
            entireSet = True  # 如果执行边没有改变，则执行所有。如果执行边界有改变，则继续边界
        # 所以该算法一定是执行所有并满足才停止。
...
def testDigits(kTup=('rbf', 10)):
    ...
    b, alphas = smoP(dataArr, labelArr, 200, 0.001, 1000, kTup)
    datMat = mat(dataArr);
    labelMat = mat(labelArr).transpose()
    svInd = nonzero(alphas.A > 0)[0]
    sVs = datMat[svInd]
